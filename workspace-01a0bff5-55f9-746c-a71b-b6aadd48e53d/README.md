...
```bash
# OPTION A — FastAPI backend (Python 3.10+)
cd backend
pip install -r requirements.txt
uvicorn main:app --host 0.0.0.0 --port 8000   # serves API + built frontend

# OPTION B — Django backend (same agents, same API, same React UI)
pip install django
cd backend_django
...
Open **http://localhost:8000** (FastAPI) or **http://localhost:8001** (Django) →
click **⚡ Load demo dataset** → tab 3 → **▶ Run Matching Pipeline**.

Both backends share the *same agent code* (`backend/agents/`) and the *same REST API* —
swap frameworks without touching the intelligence. `GET /api/health` reports which
backend is serving (`"backend": "django"`).
